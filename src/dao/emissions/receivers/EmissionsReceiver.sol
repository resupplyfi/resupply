...
contract EmissionsReceiver is EpochTracker {
    ...
    function registerReceiver(uint256 _id) external {
        require(msg.sender == address(emissionsController), "!Controller");
        receiverId = _id;
        epochFinish = block.timestamp / (epochLength + 1) * epochLength; // Next epoch
    }
