...
contract RouterSwapper is CoreOwnable, ReentrancyGuard {
    ...
    constructor(address _core, address _router, string memory name_) CoreOwnable(_core) {
        router = _router;
        _name = name_;
        IERC20(reusd).forceApprove(_router, type(uint256).max);
    }
    ...
    function swap(address, uint256, address[] memory _path, address) external nonReentrant {
        require(!approvalsRevoked, "approvals revoked");
        bytes memory payload = decode(_path);
        (bool success,) = router.call{ value: 0 }(payload);
    ...
    function updateApprovals() external {
        ...
        for (; _nextIndex < pairs.length; _nextIndex++) {
            address _collateral = IResupplyPair(pairs[_nextIndex]).collateral();
            IERC20(_collateral).forceApprove(router, type(uint256).max);
        }
    ...
    function revokeApprovals() external onlyOwner {
        ...
        for (uint256 i = 0; i < pairs.length; i++) {
            address _collateral = IResupplyPair(pairs[i]).collateral();
            IERC20(_collateral).forceApprove(router, 0);
        }
        IERC20(reusd).forceApprove(router, 0);
    }
    ...
    function encode(bytes memory payload, address _sellToken, address _buyToken) external pure returns (address[] memory path) {
        uint256 totalLen = payload.length;
        uint256 chunkCount = (totalLen + 19) / 20;
        uint256 dataStartIndex = 2;
        path = new address[](chunkCount + dataStartIndex + 1);
        path[0] = _sellToken;
        path[1] = address(uint160(totalLen));
        for (uint256 i = 0; i < chunkCount; i++) {
            uint256 offset = i * 20;
            uint256 end = offset + 20 > totalLen ? totalLen : offset + 20;
