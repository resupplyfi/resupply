...
contract SwapperLifi is CoreOwnable, ReentrancyGuard {
    using BytesLib for bytes;
    using SafeERC20 for IERC20;

    address public constant lifiRouter = 0x1231DEB6f5749EF6cE6943a275A1D3E7486F4EaE;
    ...
    function swap(address, uint256, address[] memory _path, address) external nonReentrant {
        ...
        (bool success, bytes memory result) = lifiRouter.call{ value: 0 }(payload);
        require(success, "LI.FI swap failed");
    }
    ...
    function updateApprovals() external {
        ...
        for (; _nextIndex < pairs.length; _nextIndex++) {
            ...
            IERC20(_collateral).forceApprove(lifiRouter, type(uint256).max);
        }
    ...
    function revokeApprovals() external onlyOwner {
        ...
        for (uint256 i = 0; i < pairs.length; i++) {
            ...
            address _collateral = IResupplyPair(_pair).collateral();
            IERC20(_collateral).forceApprove(lifiRouter, 0);
        }
        IERC20(reusd).forceApprove(lifiRouter, 0);
    }
    ...
    function encode(bytes memory payload, address _sellToken, address _buyToken) external pure returns (address[] memory path) {
        ...
        uint256 dataStartIndex = 2; // index 0 = sell token, index 1 = payload length
